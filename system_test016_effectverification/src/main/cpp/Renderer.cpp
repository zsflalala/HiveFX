...
#include <cassert>
#include <algorithm>
#include "Renderers/EffectVerifier.h"
...
CRenderer::CRenderer(android_app *vApp): m_pApp(vApp)
{
    ...
    m_pEffectVerifier = new CEffectVerifier(m_pApp);
    m_pRenderer       = new CSceneRenderer(m_pApp);
}
...
CRenderer::~CRenderer()
{
    ...
    if (m_pEffectVerifier) delete m_pEffectVerifier;
...
void CRenderer::renderScene()
{
    ...
    m_pRenderer->render(m_WindowWidth, m_WindowHeight);
    ...
    for (auto i = 0; i < pInputBuffer->motionEventsCount; i++)
    {
        ...
        switch (Action & AMOTION_EVENT_ACTION_MASK)
        {
            ...
            case AMOTION_EVENT_ACTION_POINTER_DOWN:
                static int Layer = 0;
                if (PointerX < m_WindowHeight / 2)
                    m_pEffectVerifier->verify();
                else
                    m_pEffectVerifier->saveTexture(m_pRenderer->getOutputFileName());
