...
void CNightSceneSequencePlayer::initBackground(const std::string &vTexturePath)
{
    int Width, Height;
    auto PictureType = EPictureType::EPictureType::PNG;
    m_pBackground = CTexture2D::loadTexture(vTexturePath, Width, Height, PictureType);
    if (!m_pBackground)
    {
...
bool CNightSceneSequencePlayer::initTextureAndShaderProgram()
{
    return CSequenceFramePlayer::initTextureAndShaderProgram("shaders/nightScene.vert", "shaders/nightScene.frag");
}
