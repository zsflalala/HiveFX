...
#include <cassert>
#include <algorithm>
#include "Renderers/NightSceneRenderer.h"
#include "Common.h"
...
CRenderer::CRenderer(android_app *vApp): m_pApp(vApp)
{
    ...
    m_pNightSceneRenderer = new CNightSceneRenderer();
}
...
CRenderer::~CRenderer()
{
    ...
    if (m_pNightSceneRenderer) delete m_pNightSceneRenderer;
}
    ...
    for (auto i = 0; i < pInputBuffer->motionEventsCount; i++)
    {
        ...
        switch (Action & AMOTION_EVENT_ACTION_MASK)
        {
            ...
            case AMOTION_EVENT_ACTION_POINTER_DOWN:
                ...
                {
                    m_pNightSceneRenderer->setRenderChannel(ERenderChannel::R);
                    LOG_INFO(TAG_KEYWORD::RENDERER_TAG, "R");
                }
                else if (PointerX < m_WindowWidth / 2.0)
                {
                    m_pNightSceneRenderer->setRenderChannel(ERenderChannel::G);
                    LOG_INFO(TAG_KEYWORD::RENDERER_TAG, "G");
                }
                else if (PointerX < m_WindowWidth / 4.0 * 3.0)
                {
                    m_pNightSceneRenderer->setRenderChannel(ERenderChannel::B);
                    LOG_INFO(TAG_KEYWORD::RENDERER_TAG, "B");
                }
                else
                {
                    m_pNightSceneRenderer->setRenderChannel(ERenderChannel::A);
