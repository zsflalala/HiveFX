#include "SnowQuantizationBicPicRenderer.h"
#include "Common.h"
#include "TimeUtils.h"
...
CSnowQuantizationBicPicRenderer::CSnowQuantizationBicPicRenderer(std::string& vConfigPath) : m_ConfigPath(vConfigPath)
{
    __initAlgorithm();
}

CSnowQuantizationBicPicRenderer::~CSnowQuantizationBicPicRenderer()
{
    if (m_pScreenQuad)
    {
        CScreenQuad::destroy();
...
void CSnowQuantizationBicPicRenderer::__initAlgorithm()
{
    ...
    int BackSeqRows = 1, BackSeqCols = 1;
    int BackFrameCount = BackGroundConfig["frames_count"].asInt();
    EPictureType::EPictureType BackPicType = EPictureType::FromString(BackGroundConfig["frames_type"].asString());
    ...
    m_pBackFramePlayer = new CSequenceFramePlayer(BackGroundPath, BackSeqRows, BackSeqCols, BackFrameCount, BackPicType);
    m_pBackFramePlayer->initTextureAndShaderProgram();
    m_pScreenQuad   = CScreenQuad::getOrCreate();
    m_LastFrameTime = CTimeUtils::getCurrentTime();
}

void CSnowQuantizationBicPicRenderer::renderScene()
{
    ...
    m_pBackFramePlayer->updateFrameAndUV(DeltaTime);
    m_pBackFramePlayer->draw(m_pScreenQuad);
